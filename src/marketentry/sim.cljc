...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (RWA, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up rw-entity-missing) ==")
    ...
    (println "== filing/submit eng-4 (rw-entity-missing -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up rdb-unverified) ==")
    ...
    (println "== filing/submit eng-5 (rdb-unverified -> HARD hold) ==")
