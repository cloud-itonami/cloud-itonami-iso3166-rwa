(ns marketentry.marketentryllm
  ...
  the RWA public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-rw-entity?=" (:has-rw-entity? e)
                        " rdb-verified?=" (:rdb-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-rw-entity? e))
                              (:has-rw-entity? e))
                          (or (not (:requires-rdb? e))
                              (:rdb-verified? e)))
                   0.9 0.3)}))
