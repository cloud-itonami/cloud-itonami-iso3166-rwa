(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Rwanda-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Rwanda-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-rw-entity?
                                       true` (near-universal for Rwanda
                                       ...
                                       Grounded in Rwandan legal entity (RDB)
                                       typically required for public awards
                                       (contracting authorities / RPPA).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(RDB/統一資格/Umucyo/RPPA登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- rw-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-rw-entity? true`, INDEPENDENTLY verify
  `:has-rw-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-rw-entity?` ground truth (most Rwanda public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-rw-entity? e))
                 (not (true? (:has-rw-entity? e))))
        [{:rule :rw-entity-missing
          :detail (str subject " はルワンダ居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- rdb-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-rdb? true`, INDEPENDENTLY check
  `:rdb-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-rdb? e))
                 (not (true? (:rdb-verified? e))))
        [{:rule :rdb-unverified
          :detail (str subject " はRDB確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (rw-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (rdb-unverified-violations request st)
