# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Rwanda

## Classification

- Repository: `cloud-itonami-iso3166-rwa`
- ISO 3166: `RWA` (Rwanda)
...
- Social impact: [:rwandan-sme-market-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Rwandan
  public contract
...
- registration walkthrough for UMUCYO ("transparency" in Kinyarwanda),
  Rwanda's e-Procurement system operated by the Rwanda Public
  Procurement Authority (RPPA), automating Government-to-Business (G2B)
  procurement interactions
- business/tax registration checklist: a company registration
  certificate with the Rwanda Development Board (RDB), automatically
  retrieved by UMUCYO via RDB integration, plus a digital certificate
  from RDB's Public Key Infrastructure (PKI) required before UMUCYO
  access
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Rwandan-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Rwanda).
