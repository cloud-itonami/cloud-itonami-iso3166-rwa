...
2. Register the client's intake: business type, target public function,
   prior filing history in Rwanda if any.
3. Run the advisor in read-only mode against UMUCYO, the RPPA-operated
   e-Procurement system.
4. Compare the checklist against the client's current documentation
   (RDB company registration, PKI digital certificate).
...
- named referral relationship with Rwandan-licensed counsel or a
...
a working referral relationship with Rwandan-licensed counsel or a
registered agent for whatever licensed representation the law of
Rwanda requires for actual public-procurement filings.
